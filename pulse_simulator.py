...
class Pulse_simulation_fixed:
    def __init__(self, Q,step_time = 10):
        duration = 8000
        detuning = [[Q[i][i]/2 ,-Q[i][i]/2] for i in range(len(Q))]
        
        for detune in detuning:
            if detune[0] > 0:
                print('detune[0] should be negative',detune[0])
                detune[0] = -0
                
        amplitude = [[0,np.median(Q[~np.eye(Q.shape[0], dtype=bool)])/2,0] for i in range(len(Q))]
        for value in range(len(Q)):
            for index in range(len(Q[0])):
                if Q[value][index] < 0 and value!=index:
                    print('Q should be positive',Q[value][index],value,index)
